  ...
  <q-page class="column no-wrap bg-service-detail q-pb-5xl">
    <q-card flat class="bg-service-card rounded-borders-none">
      <q-card-section class="q-px-xl q-py-xl">
        ...
        <div class="contact-container">
          ...
          <div @click="openWebsite" v-if="website !== ''">
    ...
    <q-card flat class="col column no-wrap bg-transparent">
      <q-card-section class="col q-py-2xl q-px-md column no-wrap">
        <q-item class="no-padding suggestions q-mb-xs">
          <q-item-section avatar>
          ...
          <q-item-section side @click="writeReview">
            <q-icon name='ti-write' color="grey-700" />
          </q-item-section>
        </q-item>
...
const categoryMap = computed(
  () =>
    categoriesList.find(category => category.value === service.value.mainCategory || category.subCategories.map(el => el.value).some(el => service.value.subCategory?.includes(el))) ||
    categoriesList.filter(c => c.value === 'other')[0]
);
...
console.log(service.value.images, '==service.value.images==')
const carouselImages = computed(
  () =>
    service.value.images?.map((image, index) => ({
      url: image?.startsWith('https://') ? image : `https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photo_reference=${image}&key=${process.env.GOOGLE_MAPS_API_KEY}`,
      index
    })) || []
);
...
const phone = computed(() => service.value.phone || '');
const website = computed(() => service.value.website || '');
...
function showDirections() {
  ...
  const desinationId = service.value.placeId;
  const destinationName = encodeURIComponent(service.value.title);

  const url = `${baseUrl}&destination_place_id=${desinationId}&destination=${destinationName}`;
...
function openWebsite() {
  openURL(website.value);
}

function writeReview() {
  router.push({name: 'write-review'})
}
...
function reportService() {
  $q.dialog({
    ...
    componentProps: {
      ...
      title: 'Report',
      message:
        'You are about to report this Service. It will be reviewed by Foodie team. Are you sure?'
    }
  }).onOk(async () => {
